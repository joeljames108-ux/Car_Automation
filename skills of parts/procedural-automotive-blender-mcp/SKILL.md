...
> [!IMPORTANT]
> **MANDATORY PREREQUISITE & QUALITY GATE**: Whenever creating, modifying, or exporting any vehicle or automotive GLB, the agent MUST activate and comply with **`glb-15mb-quality-standard`** (`.agents/skills/glb-15mb-quality-standard/SKILL.md`).
> All output models MUST achieve **Grade A (≥ 90.0%)** on `validate_glb_quality.py --strict-a` (13.5–16.5 MB, 420k–500k triangles, all 11 subsystems filled).
...
### Phase 5: Multi-Angle Visual Reference Comparison & Iterative Perfection
```text
Before exporting, set the 3D viewport to 5 standard automotive angles (Front 3/4, Rear 3/4, Side, Front, Rear) plus cockpit/macro. Capture screenshots via get_viewport_screenshot and compare against reference vehicle styling DNA. Check silhouette, stance, shutlines, optical lenses, and G2 highlight flow. Work on the mesh in Blender again and again until perfection is achieved across all views.
```

### Phase 6: Dual-Mode GLB Serialization & Post-Generation Wiring
```text
Run export batch: Save individual modular components into //exports/parts/[ObjectName].glb and the unified master model into //exports/Car_Sedan_Complete.glb. Immediately run the post-generation wiring protocol:
  python scripts/wire_vehicle_asset.py --source "exports/Car_Sedan_Complete.glb" --arch "sedan" --era "1970s" --verify-tests
This enforces the Grade A (>=90%) quality gate, copies assets to public/models/vehicles/sedan/1970s/vehicle.glb, updates public/models/vehicles/matrix_manifest.json, and verifies clean TypeScript and unit test builds.
```
