...
export const EngineStagedLoadingHUD: React.FC = () => {
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-1.5 px-4 py-2 rounded-2xl bg-slate-950/90 border border-cyan-500/40 backdrop-blur-2xl shadow-[0_0_25px_rgba(6,182,212,0.3)] select-none transition-all duration-300 pointer-events-none">
      <div className="flex items-center gap-2 text-xs font-mono font-bold text-cyan-300">
        ...
        <span className="text-cyan-400 ml-1">({progress.percentage}%)</span>
      </div>

      <div className="w-48 h-1.5 bg-slate-900 rounded-full overflow-hidden border border-cyan-500/30">
        <div
          className="h-full bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-400 transition-all duration-150 ease-out"
          style={{ width: `${progress.percentage}%` }}
        />
      </div>

      {progress.activeItem && (
        <span className="text-[9.5px] font-mono text-slate-400 truncate max-w-[200px]">
