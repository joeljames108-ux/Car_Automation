    ...
    <div className="relative w-full flex flex-col gap-3 font-mono select-none">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2.5 p-3 rounded-2xl bg-amber-950/85 border border-white/10 backdrop-blur-xl shadow-xl">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-xs font-bold font-mono tracking-wider transition-all flex items-center gap-1.5 cursor-pointer ${
              isEngineeringMode
                ...
                : "bg-white/5 text-amber-300/60 hover:text-white border-white/10 hover:bg-white/10"
            }`}
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1 rounded-xl bg-amber-950/90 border border-white/10 shadow-inner">
            <Radio size={12} className="text-emerald-400 animate-pulse" />
            <span className="text-[10px] text-amber-300/60 font-bold">HEARTBEAT IDLE</span>
      ...
      {thermalMapOpen && (
        <div className="p-4 rounded-2xl bg-gradient-to-r from-slate-950/90 via-[#130d1a]/90 to-slate-950/90 border border-rose-500/30 backdrop-blur-xl shadow-2xl animate-nh-materialize">
          <div className="flex items-center justify-between pb-2 mb-3 border-b border-rose-500/20">
            ...
            <span className="text-[10px] text-amber-300/60">MAX TOLERANCE: 950°C (INCONEL 718)</span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {[
              ...
              <div key={i} className="p-2.5 rounded-xl bg-black/40 border border-white/10 flex flex-col gap-1">
                <span className="text-[9px] text-amber-300/60 font-bold uppercase truncate">{th.label}</span>
                <span className="text-sm font-black tracking-wider" style={{ color: th.color }}>
                  {th.val}
                </span>
                <span className="text-[8px] font-bold text-amber-400/50 uppercase">{th.status}</span>
              </div>
            ))}
          </div>
        </div>
