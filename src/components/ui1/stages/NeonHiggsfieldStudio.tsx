...
function JobCard({ job }: { job: GenerationJob }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-amber-950/80/80 overflow-hidden">
      <div className="aspect-video bg-black/40 relative flex items-center justify-center">
        ...
        <span className="absolute bottom-2 left-2 px-1.5 py-0.5 rounded bg-black/60 text-[9px] font-mono uppercase text-amber-200/70">
          {job.modelId}
        </span>
      </div>
      <div className="px-2.5 py-2">
        <p className="text-[11px] font-semibold text-amber-100 truncate">{job.title}</p>
        <p className="text-[9px] text-amber-400/50 line-clamp-2 mt-0.5">{job.prompt}</p>
      </div>
    </div>
  );
}
...
function PromptBox({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <textarea
      ...
      rows={3}
      className="w-full rounded-xl bg-amber-950/80/85 border border-white/10 focus:border-sky-400/35 outline-none px-3 py-2.5 text-[11px] leading-relaxed text-amber-100 font-mono resize-none"
      spellCheck={false}
    />
  );
}
  ...
  return (
    ...
    >
      <div className="p-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 min-h-[120px]">
        {shown.length === 0 && (
          <p className="col-span-full text-center text-[11px] text-amber-400/50 py-8">
      ...
      >
        <div className="p-4 grid lg:grid-cols-[260px_1fr] gap-4">
          ...
          <div className="flex flex-col gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-1.5">Model</p>
                ...
                {HF_MODELS.filter((m) => m.kind === "image").map((m) => (
                  <button
                    ...
                    className={`text-left px-2.5 py-2 rounded-lg border transition-all ${
                      modelId === m.id
                        ? "border-sky-400/35 bg-amber-500/20"
                        : "border-white/8 bg-white/[0.03] hover:border-white/20"
                    }`}
                  >
                    <span className="text-[11px] font-bold text-slate-100">{m.label}</span>
                    <span className="block text-[9px] text-amber-400/50">{m.blurb}</span>
                  </button>
                ))}
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-1.5">Hero Paint</p>
              <input
                type="color"
                value={paintHex}
          ...
          <div className="flex flex-col gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-1.5">Scene Style</p>
                ...
                {IMAGE_STYLES.map((s) => (
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold border transition-all ${
                      styleId === s.id
                        ? "border-sky-400/35 bg-amber-500/15 text-sky-100"
                        : "border-white/10 bg-white/[0.03] text-amber-200/70 hover:border-white/25"
                    }`}
                  >
                    {s.label}
                  </button>
            ...
            <div className="flex items-center justify-between gap-3">
              <input
                ...
                className="flex-1 rounded-lg bg-amber-950/80/85 border border-white/10 focus:border-amber-500/30 outline-none px-3 py-2 text-[11px] text-amber-100 placeholder:text-slate-600"
              />
      ...
      >
        <div className="p-4 flex flex-col gap-3">
          <div className="grid md:grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-1.5">Video Model</p>
              <div className="grid grid-cols-3 gap-1.5">
                {HF_MODELS.filter((m) => m.kind === "video").map((m) => (
                  ...
                  >
                    ...
                    <span className="text-[9px] text-amber-400/50 block truncate">{m.vendor}</span>
                  </button>
                ))}
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-1.5">Viral Preset Overlay</p>
              <div className="flex flex-wrap gap-1.5">
                <button
                  onClick={() => setPresetId(null)}
                  className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold border transition-all ${
                    presetId === null ? "border-fuchsia-400/50 bg-fuchsia-400/15 text-fuchsia-100" : "border-white/10 bg-white/[0.03] text-amber-200/70"
                  }`}
                >
                  None
                </button>
                {HF_VIRAL_PRESETS.map((p) => (
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold border transition-all ${
                      presetId === p.id ? "border-fuchsia-400/50 bg-fuchsia-400/15 text-fuchsia-100" : "border-white/10 bg-white/[0.03] text-amber-200/70 hover:border-white/25"
                    }`}
                  >
                    {p.label}
                  </button>
          ...
          <div className="grid md:grid-cols-2 gap-3 mt-1">
            {shots.map((s) => (
              <div key={s.shot} className="rounded-xl border border-white/10 bg-amber-950/80/70 p-3 flex flex-col gap-2">
                ...
                <p className="text-[10px] leading-relaxed text-amber-300/60 line-clamp-3">{preset ? `${s.prompt}, ${preset.promptSuffix}` : s.prompt}</p>
              </div>
            ))}
          </div>

          <div className="flex items-center justify-between pt-1">
            <p className="text-[10px] text-amber-400/50">Tip: connect the “web” backend to open each shot pre-filled in Higgsfield Cinema Studio.</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {MUSIC_VIBES.map((v) => (
              ...
              >
                ...
                <span className="text-[9px] text-amber-400/50 leading-snug block mt-0.5">{v.desc}</span>
              </button>
            ))}
          </div>
    ...
    >
      <div className="p-4 flex flex-col gap-4">
        <div className="rounded-xl border border-amber-400/20 bg-amber-400/[0.04] p-4">
          ...
          <p className="text-[11px] italic text-amber-200/70 mt-1">“{campaign.tagline}”</p>
        </div>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-2">Channel Plan</p>
            <ul className="space-y-1.5">
              {campaign.channels.map((c) => (
                <li key={c} className="flex items-center gap-2 text-[11px] text-amber-200/70">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest text-amber-300/60 mb-2">Asset Checklist (via Higgsfield)</p>
            <ul className="space-y-1.5">
              {campaign.assetChecklist.map((a) => (
                <li key={a} className="flex items-center gap-2 text-[11px] text-amber-200/70">
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <div className="p-4 flex flex-col gap-3">
          ...
          {jobs.length > 0 && (
            <div className="rounded-xl border border-white/10 bg-amber-950/80/70 p-3">
              <div className="flex justify-between text-[10px] font-mono text-amber-300/60 mb-1.5">
              ...
              <div className="h-2 rounded-full bg-black/50 overflow-hidden">
                <div className="h-full bg-amber-500/50 transition-all duration-500" style={{ width: `${progress}%` }} />
              </div>
            </div>
          )}
        </div>
          ...
          {(
            ...
            <button
              ...
              className={`text-left px-3 py-3 rounded-xl border transition-all ${
                backend === b.id ? "border-sky-400/35 bg-amber-500/20" : "border-white/8 bg-white/[0.03] hover:border-white/20"
              }`}
            >
              <span className="text-[11px] font-bold text-slate-100 block">{b.label}</span>
              <span className="text-[9px] text-amber-400/50 leading-snug block mt-0.5">{b.desc}</span>
            </button>
          ))}
        </div>
        {backend === "api" && (
          <div className="px-4 pb-4">
            <label className="text-[10px] uppercase tracking-widest text-amber-300/60 block mb-1.5">Proxy Endpoint</label>
            <input
              value={proxyUrl}
              onChange={(e) => setProxyUrl(e.target.value)}
              placeholder="https://your-proxy.example.com/higgsfield"
              className="w-full rounded-lg bg-amber-950/80/85 border border-white/10 focus:border-amber-500/30 outline-none px-3 py-2 text-[11px] font-mono text-amber-100"
            />
            <p className="text-[9px] text-amber-400/50 mt-1.5">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
            {HF_TOOLS.map((t) => (
              <button
                ...
                className="text-left px-3 py-2.5 rounded-xl border border-white/10 bg-white/[0.03] hover:border-amber-500/30 hover:bg-amber-500/[0.06] transition-all group"
              >
                <span className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-slate-100 group-hover:text-sky-200">{t.label}</span>
                  <ExternalLink size={10} className="text-amber-400/50" />
                </span>
                <span className="text-[9px] text-amber-400/50 leading-snug block mt-0.5">{t.blurb}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-amber-300/60 uppercase tracking-[0.2em]">AI media generation · wired into APEX ENGINEER</p>
          </div>
        </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg border text-left transition-all ${
                      isActive ? "text-white" : "border-transparent text-amber-300/60 hover:bg-white/[0.05] hover:text-amber-100"
                    }`}
                    style={
                      isActive
                        ? {
                  ...
                  >
                    ...
                    <span className="ml-auto font-mono text-[9px] text-amber-400/50">
                      {t.lat >= 0 ? "+" : ""}
                      {t.lat}° / {t.lng >= 0 ? "+" : ""}
                      {t.lng}°
                    </span>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-amber-200/70 font-sans mt-0.5">{curTab.description}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg text-[10px] font-mono text-amber-200/70 hover:text-white bg-amber-950/80 hover:bg-amber-900/30 border border-white/10 transition-all flex items-center gap-1"
