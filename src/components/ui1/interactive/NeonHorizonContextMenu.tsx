...
export const NeonHorizonContextMenu: React.FC<NeonHorizonContextMenuProps> = ({
 ...
 : "text-amber-100 hover:bg-white/[0.07] hover:text-white"
 }`}
              >
                <div className="flex items-center gap-2">
                  {item.icon && <span className="text-amber-300/80">{item.icon}</span>}
                  <span>{item.label}</span>
                </div>
                {item.shortcut && (
                  <kbd className="text-[10px] nh-font-mono text-amber-300/60 opacity-60">
                    {item.shortcut}
                  </kbd>
                )}
              </button>
