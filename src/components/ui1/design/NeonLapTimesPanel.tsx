    ...
    <div className={"space-y-4 " + className}>
      <div className="rounded-xl bg-[#0e1626]/80 border border-white/[0.08] p-4 backdrop-blur-md">
        <div className="flex items-center justify-between mb-3">
          ...
          {fastest && <span className="text-[10px] font-mono text-amber-300/50">Fastest: <span className="text-emerald-400 font-bold">{fastest.trackName}</span></span>}
        </div>
          ...
          {sorted.map((lap, idx) => {
            ...
            return (
              <div key={lap.trackId} className="flex items-center gap-2 group p-1.5 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer">
                <span className="w-28 truncate text-[11px] text-amber-100/80 group-hover:text-sky-300 flex items-center gap-1.5 font-medium">
                ...
                <span className="font-mono text-[11px] text-amber-200/60 w-20 text-right">{fmtLap(lap.time)}</span>
                <ChevronRight size={10} className="text-amber-400 group-hover:text-sky-400 transition-colors" />
              </div>
            );
          })}
        </div>
