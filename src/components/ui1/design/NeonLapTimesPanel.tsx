...
export function NeonLapTimesPanel({ lapTimes, className = "" }: { lapTimes: NeonLapTimeItem[]; className?: string }) {
  ...
  return (
    <div className={"space-y-4 " + className}>
      <div className="rounded-xl bg-slate-900/80/80 border border-white/[0.08] p-4 backdrop-blur-md">
          ...
          {sorted.map((lap, idx) => {
            ...
            return (
              <div key={lap.trackId} className="flex items-center gap-2 group p-1.5 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer">
                <span className="w-28 truncate text-[11px] text-slate-300 group-hover:text-amber-300 flex items-center gap-1.5 font-medium">
