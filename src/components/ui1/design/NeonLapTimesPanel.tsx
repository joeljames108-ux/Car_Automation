          ...
          {sorted.map((lap, idx) => {
            ...
            return (
              <div key={lap.trackId} className="flex items-center gap-2 group p-1.5 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer">
                <span className="w-28 truncate text-[11px] text-amber-200/70 group-hover:text-amber-300 flex items-center gap-1.5 font-medium">
                  <MapPin size={10} className={idx === 0 ? "text-emerald-400" : "text-amber-400/50"} />{lap.trackName}
                </span>
                ...
                <span className="font-mono text-[11px] text-amber-300/60 w-20 text-right">{fmtLap(lap.time)}</span>
                <ChevronRight size={10} className="text-slate-600 group-hover:text-amber-400 transition-colors" />
              </div>
            );
          })}
        </div>
          ...
          <tbody>
            {sorted.map((lap, idx) => (
              <tr key={lap.trackId} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors cursor-pointer">
                ...
                <td className="py-2 px-2 text-amber-100 font-medium">{lap.trackName}</td>
                <td className="py-2 px-2 font-mono text-right font-bold" style={{ color: idx === 0 ? "#57a878" : "#8fb9d9" }}>{fmtLap(lap.time)}</td>
                <td className="py-2 px-2 font-mono text-right text-amber-400/50">{idx === 0 ? "--" : "+" + (lap.time - (fastest?.time || 0)).toFixed(2)}</td>
                <td className="py-2 px-2 font-mono text-right text-amber-300/60">{lap.topSpeed} km/h</td>
              </tr>
            ))}
          </tbody>
        </table></div>
