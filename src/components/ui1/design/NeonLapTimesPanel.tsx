...
export function NeonLapTimesPanel({ lapTimes, className = "" }: { lapTimes: NeonLapTimeItem[]; className?: string }) {
  ...
  return (
    <div className={"space-y-4 " + className}>
      <div className="rounded-xl bg-[#0a1628]/80 border border-white/[0.08] p-4 backdrop-blur-md">
            ...
            return (
              <div key={lap.trackId} className="flex items-center gap-2 group p-1.5 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer">
                ...
                <div className="flex-1 h-2 bg-white/[0.04] rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: Math.min(100, Math.max(10, ratio * 100)) + "%", background: idx === 0 ? "linear-gradient(90deg, #22c55e, #38bdf8)" : "linear-gradient(90deg, #0891b2, #38bdf8)" }} />
                </div>
          ...
          <tbody>
            {sorted.map((lap, idx) => (
              <tr key={lap.trackId} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors cursor-pointer">
                ...
                <td className="py-2 px-2 font-mono text-right font-bold" style={{ color: idx === 0 ? "#22c55e" : "#38bdf8" }}>{fmtLap(lap.time)}</td>
