  ...
  return (
    <div className="panel p-4 rounded-2xl space-y-3">
      <div className="flex items-center justify-between">
        <div><span className="text-[9px] font-mono text-sky-500 dark:text-sky-400 uppercase tracking-widest font-bold">Circuit Telemetry</span><h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">{trackName}</h4></div>
      ...
      <div className="h-32 w-full flex items-center justify-center"><svg viewBox="0 0 400 160" className="w-full h-full">
        <defs><linearGradient id="nCG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#38bdf8" /><stop offset="50%" stopColor="#3b82f6" /><stop offset="100%" stopColor="#a855f7" /></linearGradient></defs>
        ...
        <circle cx="90" cy="30" r="4" fill="#38bdf8" /><circle cx="240" cy="60" r="4" fill="#a855f7" /><circle cx="370" cy="50" r="4" fill="#f59e0b" />
      </svg></div>
    </div>
  );
}
...
export function NeonTelemetryGraph({ tp }: { tp: { distancePercent: number; speedKmh: number; throttlePct: number; brakePct: number }[] }) {
  ...
  return (
    <div className="panel p-4 rounded-2xl">
      <span className="text-[9px] font-mono text-sky-500 dark:text-sky-400 uppercase tracking-widest font-bold">Lap Telemetry</span>
      <div className="h-32 mt-2"><svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full h-full">
        <path d={mk(p => H - (p.speedKmh / 360) * H)} fill="none" stroke="#38bdf8" strokeWidth="2.5" vectorEffect="non-scaling-stroke" />
        <path d={mk(p => H - (p.throttlePct / 100) * H * 0.4 - 5)} fill="none" stroke="#22c55e" strokeWidth="1.5" vectorEffect="non-scaling-stroke" />
        <path d={mk(p => H - (p.brakePct / 100) * H * 0.4 - 5)} fill="none" stroke="#ef4444" strokeWidth="1.5" vectorEffect="non-scaling-stroke" />
      </svg></div>
    </div>
  );
}