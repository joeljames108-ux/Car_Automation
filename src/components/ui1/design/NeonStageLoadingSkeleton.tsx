...
export function NeonStageLoadingSkeleton({ stageName }: { stageName?: string }) {
  return (
    <NeonEntrance type="glitch" className="w-full h-full min-h-[520px] rounded-2xl bg-amber-950/80/60 border border-sky-400/15 backdrop-blur-xl p-8 flex flex-col justify-between relative overflow-hidden" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.37)" }}>
      ...
      <div className="flex items-center justify-between border-b border-white/[0.08] pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] text-amber-300/60 font-mono">{stageName ? "Loading [" + stageName.toUpperCase() + "]" : "Streaming CAD..."}</div>
          </div>
        </div>
        <div className="flex items-center gap-2 font-mono text-[10px] text-amber-400/50 bg-amber-950/80/80 px-3 py-1.5 rounded-lg border border-white/[0.06]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-8">
        {["#8fb9d9", "#9d8fc4", "#6e8cb5"].map((color, idx) => (
          <div key={idx} className="h-48 rounded-xl bg-amber-950/80/70 border border-white/[0.05] p-4 flex flex-col justify-between relative overflow-hidden">
      ...
      <div className="flex items-center justify-between text-[11px] font-mono text-amber-300/60 border-t border-white/[0.08] pt-4">
        <span>MEM: OK · SHADERS: COMPILING</span>
        <div className="w-48 h-1.5 bg-amber-950/80 rounded-full overflow-hidden border border-white/[0.08]">
