...
export function NeonStageLoadingSkeleton({ stageName }: { stageName?: string }) {
  return (
    <NeonEntrance type="glitch" className="w-full h-full min-h-[520px] rounded-2xl bg-amber-950/60/60 border border-sky-400/15 backdrop-blur-xl p-8 flex flex-col justify-between relative overflow-hidden" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.37)" }}>
      ...
      <div className="flex items-center justify-between border-b border-white/[0.08] pb-4">
        ...
        <div className="flex items-center gap-2 font-mono text-[10px] text-slate-500 bg-amber-950/60/80 px-3 py-1.5 rounded-lg border border-white/[0.06]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-8">
        {["#8fb9d9", "#9d8fc4", "#6e8cb5"].map((color, idx) => (
          <div key={idx} className="h-48 rounded-xl bg-amber-950/60/70 border border-white/[0.05] p-4 flex flex-col justify-between relative overflow-hidden">
      ...
      <div className="flex items-center justify-between text-[11px] font-mono text-slate-400 border-t border-white/[0.08] pt-4">
        ...
        <div className="w-48 h-1.5 bg-amber-950/60 rounded-full overflow-hidden border border-white/[0.08]">
