...
export function NeonStageLoadingSkeleton({ stageName }: { stageName?: string }) {
  return (
    <NeonEntrance type="glitch" className="w-full h-full min-h-[520px] rounded-2xl bg-[#0e1626]/60 border border-sky-400/15 backdrop-blur-xl p-8 flex flex-col justify-between relative overflow-hidden" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.37)" }}>
      ...
      <div className="flex items-center justify-between border-b border-white/[0.08] pb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-sky-400/10 border border-sky-400/25 flex items-center justify-center text-sky-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-8">
        {["#8fb9d9", "#9d8fc4", "#6e8cb5"].map((color, idx) => (
          <div key={idx} className="h-48 rounded-xl bg-[#0e1626]/70 border border-white/[0.05] p-4 flex flex-col justify-between relative overflow-hidden">
