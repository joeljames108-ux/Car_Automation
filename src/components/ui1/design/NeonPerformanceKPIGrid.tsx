...
function NT({ label, value, unit, icon, accent, sub }: { label: string; value: number | string; unit?: string; icon?: React.ReactNode; accent?: string; sub?: string }) {
  ...
  return (
    <div className="p-3 rounded-xl bg-[#0a1628]/80 border border-white/[0.08] backdrop-blur-md relative overflow-hidden group hover:border-white/[0.15] transition-all">
...
export function NeonPerformanceKPIGrid({ sim, metrics = ALL, className = "" }: { sim: SimResult; metrics?: NeonKPIMetricKey[]; className?: string }) {
  ...
  const D: Record<NeonKPIMetricKey, { l: string; v: number | string; u?: string; a: string; i: React.ReactNode; s?: string }> = {
    power: { l: "PEAK POWER", v: sim.peakPower, u: "hp", a: "#38bdf8", i: <Zap size={12} />, s: sim.peakPowerRpm ? "@ " + sim.peakPowerRpm + " rpm" : undefined },
    torque: { l: "PEAK TORQUE", v: sim.peakTorque, u: "Nm", a: "#f59e0b", i: <Gauge size={12} /> },
    weight: { l: "CURB WEIGHT", v: sim.weight, u: "kg", a: "#94a3b8", i: <Weight size={12} /> },
    topSpeed: { l: "TOP SPEED", v: sim.topSpeed, u: "km/h", a: "#22c55e", i: <TrendingUp size={12} /> },
    accel0_60: { l: "0-60 MPH", v: sim.accel0_60, u: "s", a: "#f97316", i: <Timer size={12} /> },
    accel0_100: { l: "0-100 KM/H", v: sim.accel0_100, u: "s", a: "#ef4444", i: <Timer size={12} /> },
    accel0_200: { l: "0-200 KM/H", v: a200, u: "s", a: "#dc2626", i: <Timer size={12} /> },
    quarterMile: { l: "QUARTER MILE", v: sim.quarterMile, u: "s", a: "#a855f7", i: <Gauge size={12} />, s: sim.quarterMileSpeed + " km/h" },
    brakingDist: { l: "BRAKING 100-0", v: sim.brakingDist, u: "m", a: "#ef4444", i: <Shield size={12} /> },
    lateralG: { l: "LATERAL G", v: sim.lateralG, u: "g", a: "#06b6d4", i: <Gauge size={12} /> },
    powerToWeight: { l: "POWER/WEIGHT", v: p2w, u: "hp/t", a: "#38bdf8", i: <Zap size={12} /> },
    cost: { l: "TOTAL COST", v: "$" + (((sim.totalCost || sim.vehicleCost || 0) / 1000).toFixed(0)) + "k", a: "#fbbf24", i: <DollarSign size={12} /> },
    dragCoeff: { l: "DRAG Cd", v: sim.dragCoeff.toFixed(3), a: "#8b5cf6", i: <Wind size={12} /> },
    downforce: { l: "DOWNFORCE", v: sim.downforce, u: "kg", a: "#22d3ee", i: <Wind size={12} /> },
    reliability: { l: "RELIABILITY", v: (sim.reliability * 100).toFixed(0), u: "%", a: "#22c55e", i: <Shield size={12} /> },
  };
  return (
