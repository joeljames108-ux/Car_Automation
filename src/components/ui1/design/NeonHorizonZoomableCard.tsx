...
export function NeonHorizonZoomableCard({
    ...
    >
      {allowZoom && (
        <button
          ...
          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-all duration-200 bg-black/60 border border-white/15 text-amber-200/70 p-1.5 rounded-full shadow-lg z-20 hover:bg-white/10 hover:text-white hover:scale-110 active:scale-95 cursor-pointer backdrop-blur-md"
          title="Expand Panel"
        >
          <Maximize2 size={12} />
        </button>
 ...
 modalActive ? "scale-100 translate-y-0" : "scale-95 translate-y-4"
          ...
          >
            <div className="rounded-3xl bg-amber-950/80/95 backdrop-blur-3xl border border-white/12 shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden p-6">
              <div className="w-full flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                <button
                  onClick={closeZoomModal}
                  className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/[0.06] text-amber-100 border border-white/12 text-xs font-mono font-bold hover:bg-white/[0.12] transition-all active:scale-95 cursor-pointer"
                >
                  <ArrowLeft size={14} /> Back
                </button>
                <div className="flex items-center gap-2 text-xs font-mono font-bold uppercase tracking-widest text-amber-200/70">
                  <Sparkles size={14} className={glowColors.text} />
                  {title || "Neon Horizon Panel"}
                </div>
                <button
                  onClick={closeZoomModal}
                  className="p-1.5 rounded-full text-amber-300/60 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              {(subtitle || badge) && (
                <div className="w-full flex items-center justify-between mb-3 text-xs font-mono">
                  {subtitle && <span className="text-amber-300/60">{subtitle}</span>}
                  {badge && (
