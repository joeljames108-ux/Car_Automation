...
const DifficultyStars = memo(function DifficultyStars({ value }: { value: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} size={12} className={i <= value ? "text-yellow-400 fill-yellow-400" : "text-amber-500"} />
      ))}
    </div>
  );
});
        ...
        {(Object.keys(CATEGORY_LABELS) as MotorsportCategory[]).map(cat => (
          <button
            ...
            className={`px-2 py-2 rounded-xl text-xs font-semibold transition-all border cursor-pointer ${
              guideCategory === cat ? CATEGORY_COLORS[cat] : "bg-base-850 border-base-800 text-amber-200/60 hover:border-base-700"
            }`}
          >
            {CATEGORY_LABELS[cat]}
          </button>
      ...
      <div className="glass-panel p-5 relative overflow-hidden border-amber-500/20">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-3">
          <div>
            ...
            <div className="flex items-center gap-4 mt-2 flex-wrap">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-amber-200/60">Difficulty:</span>
                <DifficultyStars value={guide.difficulty} />
                <span className="text-[10px] text-amber-100/80 font-medium">({DIFFICULTY_LABELS[guide.difficulty]})</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-amber-200/60">Prestige:</span>
          ...
          <div className="bg-base-950/80 p-3 rounded-xl border border-white/10 text-right shrink-0">
            <div className="text-[10px] text-amber-200/60">Estimated Budget</div>
            <div className="text-sm font-mono font-bold text-amber-300">
              ${(guide.budgetRange[0] / 1_000_000).toFixed(0)}M – ${(guide.budgetRange[1] / 1_000_000).toFixed(0)}M
            </div>
          </div>
        </div>
        <p className="text-xs text-amber-200/60 leading-relaxed">{guide.description}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Iconic Races */}
        <div className="glass-panel p-4 border-white/5">
          <h4 className="text-xs font-semibold text-amber-100/80 uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <Award size={14} className="text-yellow-400" /> Iconic Races & Circuits
          </h4>
          <div className="space-y-1.5">
            {guide.iconicRaces.map(r => (
              <div key={r} className="flex items-center gap-2 text-xs text-amber-100/80 bg-base-900/50 px-2.5 py-1.5 rounded-lg">
                <ChevronRight size={12} className="text-amber-400" /> {r}
              </div>
            ))}
          </div>
          <div className="mt-3 pt-3 border-t border-white/5">
            <div className="text-[10px] text-amber-300/50 mb-1.5">Real-World Inspiration</div>
            <div className="flex flex-wrap gap-1">
              {guide.realWorldSeries.map(s => (
                <span key={s} className="text-[10px] px-2 py-0.5 bg-base-850 border border-base-800 rounded-full text-amber-100/80">{s}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 border-white/5">
          ...
          <div className="space-y-2.5 text-xs">
            <div className="flex justify-between bg-base-900/50 p-2 rounded-lg">
              <span className="text-amber-200/60">Power Window</span>
              <span className="text-amber-50 font-mono font-bold">{guide.recommendedSpecs.powerRange[0]}–{guide.recommendedSpecs.powerRange[1]} HP</span>
            </div>
            <div className="flex justify-between bg-base-900/50 p-2 rounded-lg">
              <span className="text-amber-200/60">Target Weight</span>
              <span className="text-amber-50 font-mono font-bold">{guide.recommendedSpecs.weightRange[0]}–{guide.recommendedSpecs.weightRange[1]} kg</span>
            </div>
            <div className="flex justify-between bg-base-900/50 p-2 rounded-lg">
              <span className="text-amber-200/60">Aero Importance</span>
            ...
            <div className="flex justify-between bg-base-900/50 p-2 rounded-lg">
              <span className="text-amber-200/60">Reliability Priority</span>
              <span className={`capitalize font-bold ${
      ...
      <div className="glass-panel p-5 border-white/5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
          <div className="bg-base-950 rounded-lg p-2.5 text-center border border-white/5">
            <div className="text-[10px] text-amber-300/50 mb-1">Min Weight</div>
            <div className="font-mono text-sm font-bold text-amber-50">{reg.minWeightKg} kg</div>
          </div>
          <div className="bg-base-950 rounded-lg p-2.5 text-center border border-white/5">
            <div className="text-[10px] text-amber-300/50 mb-1">Max Power</div>
            <div className="font-mono text-sm font-bold text-amber-50">{reg.maxPowerHp} HP</div>
          </div>
          <div className="bg-base-950 rounded-lg p-2.5 text-center border border-white/5">
            <div className="text-[10px] text-amber-300/50 mb-1">Pit Stops</div>
            <div className="font-mono text-sm font-bold text-amber-50">{reg.mandatoryPitStops}+</div>
          </div>
          <div className="bg-base-950 rounded-lg p-2.5 text-center border border-white/5">
            <div className="text-[10px] text-amber-300/50 mb-1">Fuel Cap</div>
            <div className="font-mono text-sm font-bold text-amber-50">{reg.fuelCapacityLiters}L</div>
          </div>
        </div>
        ...
        <div className="bg-base-950/80 p-4 rounded-xl border border-white/10">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-amber-50">Current Design Compliance: {CATEGORY_LABELS[guideCategory]}</span>
            </div>
            ...
            {compliance.checks.map((c, i) => (
              <div key={i} className="flex items-center justify-between text-xs bg-base-900/60 p-2 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-amber-100/80">{c.label}</span>
                </div>
                <span className="text-amber-200/60 text-[11px] font-mono">{c.detail}</span>
              </div>
            ))}
          </div>
        </div>
