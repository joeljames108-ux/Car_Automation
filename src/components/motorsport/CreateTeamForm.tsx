    ...
    <div className="glass-panel p-5 border-accent-500/30 racing-stripes relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-2 mb-5">
          {steps.map((_, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                ...
                "bg-base-800 text-amber-400 border border-base-700"
              }`}>{i < step ? "✓" : i + 1}</div>
              {i < steps.length - 1 && (
          ...
          <div className="ml-3">
            <div className="text-sm font-semibold text-amber-50">{steps[step].title}</div>
            <div className="text-[10px] text-amber-300/50">{steps[step].subtitle}</div>
          </div>
        </div>
          ...
          <div className="space-y-3 animate-fade-in-up">
            <div>
              ...
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                ...
                autoFocus
                className="w-full bg-base-850 border border-base-700 rounded-lg px-3 py-2.5 text-sm text-amber-50 focus:border-accent-500 focus:outline-none focus:ring-1 focus:ring-accent-500/30 transition-all" />
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={onClose} className="px-4 py-2 rounded-lg text-xs text-amber-300/50 hover:text-amber-100/80 hover:bg-base-800 transition-all cursor-pointer">Cancel</button>
              <button onClick={() => {
                if (form.name.trim()) {
                  playHMIClickSound();
                  setStep(1);
          ...
          <div className="space-y-3 animate-fade-in-up">
              ...
              {(Object.keys(CATEGORY_LABELS) as MotorsportCategory[]).map(cat => (
                <button key={cat} onClick={() => {
                  ...
                  className={`px-3 py-3 rounded-xl text-xs font-medium transition-all border card-hover cursor-pointer ${
                    form.category === cat ? `${CATEGORY_COLORS[cat]} shadow-lg` : "bg-base-850 border-base-800 text-amber-200/60 hover:border-base-700"
                  }`}>
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => {
                playHMIClickSound();
                setStep(0);
              }} className="px-4 py-2 rounded-lg text-xs text-amber-300/50 hover:text-amber-100/80 hover:bg-base-800 transition-all cursor-pointer">Back</button>
              <button onClick={() => {
                playHMIClickSound();
                setStep(2);
              }}
        ...
        {step === 2 && (
          <div className="space-y-4 animate-fade-in-up">
            <div>
              ...
              <div className="flex justify-between text-[10px] text-amber-400 mt-1 px-0.5">
                <span>$5M</span>
                <span>$100M</span>
              </div>
            </div>
            <div>
              ...
              <select value={form.baseVehicleId || ""} onChange={e => setForm(f => ({ ...f, baseVehicleId: e.target.value || null }))}
                className="w-full bg-base-850 border border-base-700 rounded-lg px-3 py-2 text-sm text-amber-50 focus:border-accent-500 focus:outline-none transition-all">
                <option value="">None (clean-sheet design)</option>
                {company.garage.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
              </select>
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setStep(1)} className="px-4 py-2 rounded-lg text-xs text-amber-300/50 hover:text-amber-100/80 hover:bg-base-800 transition-all">Back</button>
