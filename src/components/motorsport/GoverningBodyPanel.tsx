      ...
      <div className="glass-panel p-6 border-amber-500/25 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-xl font-bold text-amber-50">FIA International Motorsport Authority</h2>
              <p className="text-xs text-amber-200/60">Regulating technical specifications, race calendars, budget caps & pit protocols</p>
            </div>
          </div>

          {/* Category selector */}
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs font-mono text-amber-200/60">Championship:</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          <div className="bg-base-950/80 rounded-xl p-4 border border-white/5">
            <div className="text-[10px] font-mono text-amber-300/50 uppercase mb-1">Max Power Output</div>
            <div className="text-xl font-black font-mono text-amber-300">{reg.maxPowerHp} HP</div>
            <div className="text-[10px] text-amber-300/50 mt-1">Enforced by fuel flow restrictors</div>
          </div>
          <div className="bg-base-950/80 rounded-xl p-4 border border-white/5">
            <div className="text-[10px] font-mono text-amber-300/50 uppercase mb-1">Minimum Car Weight</div>
            <div className="text-xl font-black font-mono text-amber-50">{reg.minWeightKg} kg</div>
            <div className="text-[10px] text-amber-300/50 mt-1">Post-race ballast scrutiny</div>
          </div>
          <div className="bg-base-950/80 rounded-xl p-4 border border-white/5">
            <div className="text-[10px] font-mono text-amber-300/50 uppercase mb-1">Budget Cap Mandate</div>
            <div className="text-xl font-black font-mono text-emerald-400">
              {reg.maxBudgetCap > 0 ? `$${(reg.maxBudgetCap / 1e6).toFixed(0)}M` : "NO CAP"}
            </div>
            <div className="text-[10px] text-amber-300/50 mt-1">Financial fair play audit</div>
          </div>
          <div className="bg-base-950/80 rounded-xl p-4 border border-white/5">
            <div className="text-[10px] font-mono text-amber-300/50 uppercase mb-1">Mandatory Pit Stops</div>
            <div className="text-xl font-black font-mono text-amber-400">{reg.mandatoryPitStops} Stops</div>
            <div className="text-[10px] text-amber-300/50 mt-1">Multi-compound tire rules</div>
          </div>
        </div>

        {/* Official Regulation Mandates List */}
        <div className="bg-base-950/60 rounded-xl p-4 border border-white/5 space-y-3">
          <h4 className="text-xs font-bold text-amber-50 uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
            <div className="bg-base-900/60 p-3 rounded-lg border border-base-800 flex items-center justify-between">
              <span className="text-amber-100/80">Balance of Performance (BoP):</span>
              <span className={`font-semibold ${reg.bopEnabled ? "text-emerald-400" : "text-amber-300/50"}`}>
                {reg.bopEnabled ? "ACTIVE (STRICT)" : "DISABLED"}
              </span>
            </div>
            <div className="bg-base-900/60 p-3 rounded-lg border border-base-800 flex items-center justify-between">
              <span className="text-amber-100/80">Hybrid / EV Architecture:</span>
              <span className={`font-semibold ${reg.evRequirement ? "text-amber-400" : "text-amber-300/50"}`}>
                {reg.evRequirement ? "MANDATORY" : "OPTIONAL"}
              </span>
            </div>
            <div className="bg-base-900/60 p-3 rounded-lg border border-base-800 flex items-center justify-between">
              <span className="text-amber-100/80">Tire Sets Per Race Weekend:</span>
              <span className="font-semibold text-amber-50">{reg.maxTireSetsPerRace} Sets Max</span>
            </div>
            <div className="bg-base-900/60 p-3 rounded-lg border border-base-800 flex items-center justify-between">
              <span className="text-amber-100/80">Air Intake Restrictor Plate:</span>
              <span className={`font-semibold ${reg.restrictorPlate ? "text-amber-400" : "text-amber-300/50"}`}>
