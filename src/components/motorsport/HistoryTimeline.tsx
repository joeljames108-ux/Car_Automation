              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm font-bold text-amber-50">Season {season.season}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {isExpanded ? <ChevronUp size={14} className="text-amber-300/50" /> : <ChevronDown size={14} className="text-amber-300/50" />}
                  </div>
                </div>

                {/* Summary stats */}
                <div className="flex gap-4 mt-2 text-[10px]">
                  <span className="text-amber-200/60">{season.points}<span className="text-amber-400 ml-0.5">pts</span></span>
                  <span className="text-ok-400">{season.wins}<span className="text-amber-400 ml-0.5">W</span></span>
                  <span className="text-accent-300">{season.podiums}<span className="text-amber-400 ml-0.5">P</span></span>
                  <span className="text-amber-400">{season.fastestLaps}<span className="text-amber-400 ml-0.5">FL</span></span>
                  {season.dnfs > 0 && <span className="text-danger-400">{season.dnfs}<span className="text-amber-400 ml-0.5">DNF</span></span>}
                </div>
                ...
                {isExpanded && (
                  <div className="mt-4 pt-3 border-t border-base-800/50 animate-fade-in-up">
                    ...
                    <div className="mb-4">
                      <div className="text-[10px] text-amber-300/50 uppercase tracking-wider mb-2 font-semibold">Race Results</div>
                      <div className="flex flex-wrap gap-1">
                        {season.raceResults.map(r => (
                          <div key={r.round} className={`text-[10px] px-2.5 py-1.5 rounded-lg font-mono ${
                            ...
                            "bg-base-850/50 text-amber-300/50 border border-base-800"
                          }`}>
                    ...
                    {season.standings.length > 0 && (
                      <div>
                        <div className="text-[10px] text-amber-300/50 uppercase tracking-wider mb-2 font-semibold">Final Standings</div>
                        <div className="space-y-1">
                          {season.standings.slice(0, 6).map(s => (
                            <div key={s.teamId} className={`flex items-center gap-2 text-xs px-2 py-1.5 rounded ${
                              s.isPlayer ? "bg-accent-500/10 text-accent-300" : "text-amber-200/60"
                            }`}>
                              <span className={`w-5 font-mono font-bold ${
                                s.position === 1 ? "text-yellow-400" : s.position === 2 ? "text-amber-100/80" : s.position === 3 ? "text-amber-600" : "text-amber-400"
                              }`}>{s.position}</span>
                              <span className="flex-1">{s.teamName}</span>
                              <span className="font-mono text-amber-100/80">{s.points}pts</span>
                              <span className="text-[10px] text-amber-400">{s.wins}W</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-center">
                      <div className="bg-base-850/50 rounded-lg p-2">
                        <div className="text-xs font-mono font-bold text-amber-100/80">{season.techPointsEarned}</div>
                        <div className="text-[9px] text-amber-400">Tech Earned</div>
                      </div>
                      <div className="bg-base-850/50 rounded-lg p-2">
                        <div className="text-xs font-mono font-bold text-amber-100/80">{season.polePositions}</div>
                        <div className="text-[9px] text-amber-400">Pole Pos</div>
                      </div>
                      <div className="bg-base-850/50 rounded-lg p-2">
                        <div className={`text-xs font-mono font-bold ${season.penaltyPoints > 0 ? "text-warn-400" : "text-amber-100/80"}`}>{season.penaltyPoints}</div>
                        <div className="text-[9px] text-amber-400">Penalties</div>
                      </div>
                    </div>
                  </div>
                )}
