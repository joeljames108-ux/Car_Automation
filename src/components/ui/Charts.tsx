...
export function RadarChart({ axes, max = 10, size = 220 }: { axes: RadarAxis[]; max?: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} className="overflow-visible">
      ...
      <path d={polyPath} fill="url(#radarFill)" stroke="#fbbf24" strokeWidth="2" strokeLinejoin="round" />
      {axes.map((a, i) => {
        const [x, y] = point(i, a.value);
        return <circle key={i} cx={x} cy={y} r="3" fill="#fde68a" stroke="#0b0f17" strokeWidth="1.5" />;
      })}
      {axes.map((a, i) => {
...
export function RadialGauge({ value, max = 10, label, size = 140 }: { value: number; max?: number; label?: string; size?: number }) {
  ...
  const color = pct >= 0.85 ? "#22c55e" : pct >= 0.7 ? "#fbbf24" : pct >= 0.55 ? "#f59e0b" : "#ef4444";
  return (
...
export function BarCompare({ rows, labels, highlight }: {
  ...
  const max = 10;
  const colors = ["#fbbf24", "#64748b", "#475569"];
  return (
...
export function Sparkline({ values, width = 120, height = 32, color = "#fbbf24" }: { values: number[]; width?: number; height?: number; color?: string }) {
    ...
    <div className="bg-base-950/60 p-4 rounded-xl border border-white/10 relative overflow-hidden flex flex-col gap-3">
      ...
      <div className="h-32 w-full relative flex items-center justify-center">
        <svg viewBox="0 0 400 160" className="w-full h-full">
          ...
          {/* Sector dots */}
          <circle cx="90" cy="30" r="4" fill="#fbbf24" />
          ...
          <text x="90" y="20" fill="#fbbf24" fontSize="8" fontFamily="monospace">T1 Hairpin</text>
          <text x="240" y="48" fill="#f59e0b" fontSize="8" fontFamily="monospace">T4 Chicane</text>
...
export function TelemetryGraph({ telemetryPoints }: {
  ...
  return (
    <div className="bg-base-950/60 p-4 rounded-xl border border-white/10 space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-mono text-amber-400 uppercase tracking-widest font-bold">REAL-TIME LAP TELEMETRY SPEEDS</span>
        <div className="flex items-center gap-3 text-[10px] font-mono">
          <span className="text-amber-400 font-bold">Speed (km/h)</span>
      ...
      <div className="h-32 w-full relative">
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full h-full">
          ...
          {/* Speed trace */}
          <path d={speedLine} fill="none" stroke="#fbbf24" strokeWidth="2.5" vectorEffect="non-scaling-stroke" />
...
export function SectorTimesBarChart({ s1, s2, s3, bestS1 = 24.1, bestS2 = 32.4, bestS3 = 21.8 }: {
  ...
  const getSectorColor = (val: number, best: number) => {
    if (val <= best) return { bg: "bg-amber-500/20", border: "border-amber-500/40", text: "text-amber-300", badge: "PURPLE (SESSION BEST)" };
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className={`p-2.5 rounded-lg border ${c1.bg} ${c1.border} space-y-1`}>
          ...
          <div className="w-full bg-base-900 h-1.5 rounded-full overflow-hidden">
            <div className="h-full bg-amber-400 rounded-full" style={{ width: `${(bestS1 / s1) * 100}%` }} />
          </div>
        </div>
    ...
    <div className="bg-base-950/60 p-4 rounded-xl border border-white/10 space-y-3 relative overflow-hidden">
      ...
      <div className="h-28 w-full relative flex items-center justify-center">
        <svg viewBox="0 0 360 110" className="w-full h-full">
          ...
          {/* Front Wing Vector */}
          <line x1="60" y1="20" x2="60" y2="60" stroke="#f59e0b" strokeWidth="2.5" />
          <polygon points="56,58 60,67 64,58" fill="#f59e0b" />
          <text x="45" y="15" fill="#f59e0b" fontSize="8" fontFamily="monospace">FRONT DOWNFORCE ({aeroBalancePct}%)</text>

          {/* Rear Wing Vector */}
          <line x1="320" y1="10" x2="320" y2="40" stroke="#f59e0b" strokeWidth="2.5" />
          <polygon points="316,38 320,47 324,38" fill="#f59e0b" />
          <text x="270" y="10" fill="#f59e0b" fontSize="8" fontFamily="monospace">REAR WING ({100 - aeroBalancePct}%)</text>
