...
export const StageLoadingSkeleton: React.FC<StageLoadingSkeletonProps> = ({ stageName }) => {
  return (
    <div className="w-full h-full min-h-[520px] rounded-2xl bg-base-900/60 border border-cyan-500/20 backdrop-blur-xl p-8 flex flex-col justify-between relative overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.37)]">
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-cyan-500/10 border border-cyan-400/30 flex items-center justify-center animate-pulse text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.25)]">
            <Cpu size={20} className="animate-spin" style={{ animationDuration: '3s' }} />
          </div>
          <div>
            <div className="text-xs font-mono font-bold text-cyan-300 tracking-wider flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-8">
        <div className="h-48 rounded-xl bg-base-950/70 border border-white/5 p-4 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <div className="h-2 w-3/5 bg-cyan-500/20 rounded animate-pulse" />
          </div>
          <div className="h-8 w-full bg-cyan-500/10 rounded border border-cyan-500/20 animate-pulse" />
        </div>
        ...
        <div className="h-48 rounded-xl bg-base-950/70 border border-white/5 p-4 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <div className="h-2 w-2/3 bg-purple-500/20 rounded animate-pulse" />
          </div>
          <div className="h-8 w-full bg-purple-500/10 rounded border border-purple-500/20 animate-pulse" />
        </div>
      ...
      <div className="flex items-center justify-between text-[11px] font-mono text-slate-400 border-t border-white/10 pt-4">
        ...
        <div className="w-48 h-1.5 bg-base-950 rounded-full overflow-hidden border border-white/10">
          <div className="h-full bg-gradient-to-r from-cyan-400 via-sky-400 to-purple-500 w-2/3 rounded-full animate-pulse" />
        </div>
      </div>
    </div>
  );
