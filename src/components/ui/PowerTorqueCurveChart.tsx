...
export const PowerTorqueCurveChart: React.FC<PowerTorqueCurveChartProps> = ({
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      {showLegend && (
        <div className="flex justify-between text-[10px] text-amber-300/50 mt-1.5 px-1">
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-3 bg-amber-400 rounded-sm" />
            <span className="font-mono text-amber-100/80">Power (hp)</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-3 bg-amber-500 rounded-sm" />
            <span className="font-mono text-amber-100/80">Torque (Nm)</span>
          </span>
        </div>
      )}
    </div>
