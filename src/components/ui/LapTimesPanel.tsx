  ...
  const renderBars = () => (
    <Section title="Circuit Comparison" icon={<Map size={16} />}>
      <div className="space-y-2 text-xs">
        <div className="flex justify-between text-amber-200/60 border-b border-base-800 pb-1">
          <span>
            Fastest: <strong className="text-accent-400">{fastest?.trackName}</strong> ({formatLap(fastest?.time)})
          </span>
          <span>
            Slowest: <strong className="text-amber-100/80">{slowest?.trackName}</strong> ({formatLap(slowest?.time)})
          </span>
        </div>
        <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
          {sorted.map((lap) => {
            ...
            return (
              ...
              >
                <span className="w-24 truncate text-amber-100/80 group-hover:text-accent-300 transition-colors flex items-center gap-1">
                ...
                <span className="font-mono text-amber-200/60 w-16 text-right">{formatLap(lap.time)}</span>
              </div>
            );
          })}
        </div>
          ...
          <tbody>
            {sorted.map((lap, idx) => {
              ...
              return (
                ...
                >
                  <td className="py-2 px-2 font-mono text-amber-300/50">{idx + 1}</td>
                  <td className="py-2 px-2 text-amber-50 group-hover:text-accent-300 font-medium flex items-center gap-1.5">
                  ...
                  <td className="py-2 px-2 font-mono text-right text-amber-300/50">
                  ...
                  <td className="py-2 px-2 font-mono text-right text-amber-200/60">{lap.avgSpeed} km/h</td>
                  <td className="py-2 px-2 font-mono text-right text-amber-300/50">
