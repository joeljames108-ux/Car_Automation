    ...
    <div className="space-y-6 text-amber-50">
      ...
      <div className="bg-amber-900/40 backdrop-blur-xl p-6 rounded-2xl border border-amber-800/30 flex items-center justify-between shadow-xl">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center space-x-2 transition-all ${
              exhaustValveOpen
                ...
                : "bg-amber-800/35 text-amber-200/60"
            }`}
          >
          ...
          >
            {ancActive ? <VolumeX className="w-4 h-4 text-emerald-400" /> : <Volume2 className="w-4 h-4 text-amber-200/60" />}
      ...
      <div className="bg-amber-900/40 p-5 rounded-2xl border border-amber-800/30 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          <div className="flex justify-between text-xs text-amber-100/80 mb-2 font-mono">
        ...
        <div>
          ...
          <input
            ...
            className="w-full accent-indigo-500 bg-amber-950/80 rounded-lg cursor-pointer"
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          ...
          <div className="text-2xl font-mono font-black text-amber-400 mt-2">
            {acousticOutput.finalCabinDba} <span className="text-xs text-amber-300/50">dBA</span>
          </div>
          <div className="text-[11px] text-amber-300/50 mt-1">Raw Uncancelled: {acousticOutput.orderSweep.totalSoundPressureDba} dBA</div>
        </div>
        ...
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          ...
          <div className="text-2xl font-mono font-black text-amber-400 mt-2">
            {acousticOutput.psychoacoustics.zwickerLoudnessSones} <span className="text-xs text-amber-300/50">Sones</span>
          </div>
          <div className="text-[11px] text-amber-300/50 mt-1">Aures Sharpness: {acousticOutput.psychoacoustics.auresSharpnessAcums} Acums</div>
        </div>
        ...
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          ...
          <div className="text-[11px] text-amber-300/50 mt-1">Roughness: {acousticOutput.psychoacoustics.asperRoughnessAsper} Asper</div>
        </div>

        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          <div className="text-xs text-amber-200/60 font-semibold">CABIN LUXURY RATING</div>
          <div className="text-sm font-black text-emerald-400 mt-2">{acousticOutput.psychoacoustics.cabinLuxuryRating}</div>
          <div className="text-[11px] text-amber-200/60 mt-1 line-clamp-2">{acousticOutput.psychoacoustics.perceivedAcousticSummary}</div>
        </div>
      </div>

      {/* Acoustic Order Spectrum Table */}
      <div className="bg-amber-900/40 p-5 rounded-2xl border border-amber-800/30 space-y-3">
        <h3 className="text-sm font-bold text-amber-50">ACOUSTIC ORDER HARMONICS SPECTRUM</h3>
        <div className="space-y-2">
          {acousticOutput.orderSweep.orderSpectrum.map((ord, idx) => (
            <div key={idx} className="bg-amber-950/80 p-3 rounded-xl border border-amber-800/30 flex items-center justify-between">
              <div>
                <div className="text-xs font-bold text-amber-50">{ord.description}</div>
                <div className="text-[10px] text-amber-300/50">Source: {ord.sourceType}</div>
              </div>
              <div className="text-right font-mono">
                <div className="text-xs font-bold text-amber-400">{ord.frequencyHz} Hz</div>
                <div className="text-[11px] text-amber-200/60">{ord.amplitudeDba} dBA</div>
              </div>
            </div>
          ))}
        </div>
