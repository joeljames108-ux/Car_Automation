...
export const LiveryStudio: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      {/* Studio Header */}
      <div className="glass-panel p-6 border-pink-500/20 bg-gradient-to-r from-slate-900 via-slate-900/90 to-pink-950/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="text-[10px] text-slate-400 uppercase tracking-wider">{l.titleSponsorName}</div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
          {PRESET_PALETTES.map((palette) => (
            ...
            >
              ...
              <div className="text-[10px] font-semibold text-slate-300 truncate group-hover:text-pink-300">
                {palette.name}
              </div>
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-3">
          <label className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center justify-between">
            ...
            <span className="font-mono text-xs text-slate-400">{l.primaryColorHex}</span>
          </label>
          <div className="flex items-center gap-3">
            <input
              type="color"
            ...
            <input
              ...
              value={l.primaryColorHex}
              onChange={(e) => updateLivery({ primaryColorHex: e.target.value })}
              className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 font-mono focus:outline-none focus:border-pink-500"
            />
          </div>
          <p className="text-[11px] text-slate-500">Applies to the main survival cell, nose cone, and sidepod upper surfaces.</p>
        </div>
        ...
        <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-3">
          <label className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center justify-between">
            ...
            <span className="font-mono text-xs text-slate-400">{l.secondaryColorHex}</span>
          </label>
          <div className="flex items-center gap-3">
            <input
              type="color"
          ...
          <p className="text-[11px] text-slate-500">Applies to front/rear wing endplates, DRS flap, and shark fin stripe.</p>
        </div>
        ...
        <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-3">
          ...
          <select
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-pink-500"
          >
        ...
        <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-2">
          <div className="flex justify-between items-center text-xs">
            <span className="font-bold text-slate-300 uppercase tracking-wider">Driver Competition Number</span>
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 font-mono focus:outline-none focus:border-pink-500"
          />
          <p className="text-[11px] text-slate-500">Number #1 is reserved for the defending World Drivers' Champion.</p>
        </div>

        {/* 5. Title Sponsor Name */}
        <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-2">
          <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Title Sponsor Name</span>
          <input
            type="text"
            value={l.titleSponsorName}
            onChange={(e) => updateLivery({ titleSponsorName: e.target.value })}
            className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 font-semibold focus:outline-none focus:border-pink-500"
          />
          <p className="text-[11px] text-slate-500">Displayed on the sidepods and rear wing mainplane.</p>
        </div>
      </div>
    </div>
  );
