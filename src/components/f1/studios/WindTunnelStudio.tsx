    ...
    <div className="space-y-6 animate-fade-in-up">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-2xl font-black font-mono text-amber-400">
              {liftToDragRatio} <span className="text-xs text-amber-200/60 font-normal">L/D</span>
            </div>
            <div className="text-[10px] text-amber-200/60 uppercase tracking-wider">Aero Efficiency Index</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30 text-center">
          <div className="text-[10px] text-amber-300/50 uppercase tracking-wider mb-1">Tunnel Wind Speed</div>
          <div className="font-mono text-2xl font-bold text-amber-50">{windSpeedKmh} <span className="text-xs text-amber-200/60">km/h</span></div>
          <div className="text-[11px] text-amber-400 mt-1">FIA 180 km/h ATR Cap</div>
        </div>

        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30 text-center">
          <div className="text-[10px] text-amber-300/50 uppercase tracking-wider mb-1">Measured Downforce</div>
          <div className="font-mono text-2xl font-bold text-amber-300">{dynamicDownforce} <span className="text-xs text-amber-200/60">kg</span></div>
          <div className="text-[11px] text-amber-200/60 mt-1">{aero.frontAeroBalancePercent}% Front Bias</div>
        </div>

        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30 text-center">
          <div className="text-[10px] text-amber-300/50 uppercase tracking-wider mb-1">Measured Drag</div>
          <div className="font-mono text-2xl font-bold text-amber-400">{dynamicDrag} <span className="text-xs text-amber-200/60">kg</span></div>
          <div className="text-[11px] text-amber-200/60 mt-1">CdA ~ 1.18 m²</div>
        </div>

        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30 text-center">
          <div className="text-[10px] text-amber-300/50 uppercase tracking-wider mb-1">Porpoising Risk Index</div>
          <div className="font-mono text-2xl font-bold text-ok-400">
            {aero.floorVenturiThroatHeightMm < 13 ? "HIGH (84%)" : "LOW (12%)"}
          </div>
          <div className="text-[11px] text-amber-200/60 mt-1">{aero.floorVenturiThroatHeightMm}mm Throat Clearance</div>
        </div>
      </div>

      {/* Interactive Tunnel Controls */}
      <div className="bg-amber-900/40 p-5 rounded-xl border border-amber-800/30 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-xs font-bold text-amber-50 uppercase tracking-wider">Tunnel Wind Velocity Sweep</span>
            <div className="text-xs text-amber-200/60">Slide to test aero load scaling across speed envelopes.</div>
          </div>
          <input
            type="range"
            min="60"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              tunnelRunning
                ...
                : "bg-amber-800/35 hover:bg-amber-700/40 text-amber-50"
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-semibold border transition-all cursor-pointer ${
              smokeStreams
                ...
                : "bg-amber-800/35 border-amber-700/30 text-amber-200/60 hover:text-amber-50"
            }`}
          >
