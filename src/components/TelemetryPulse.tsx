...
// Static engineering stats — a snapshot of the current design, not a live feed.
const TELEMETRY_GAUGES: GaugeData[] = [
    ...
    <div className="panel bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-2xl p-4 shadow-[0_0_40px_rgba(0,0,0,0.6)] relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-3 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="text-[9px] font-mono font-bold text-amber-400 uppercase tracking-widest">TELEMETRY</span>
          </div>
        </div>
        <span className="flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-mono font-bold bg-slate-800 text-slate-400 border border-slate-700">
          DESIGN SNAPSHOT
        </span>
      </div>
      ...
      <div className="grid grid-cols-6 gap-1 relative z-10">
        {TELEMETRY_GAUGES.map(g => (
          <StaticGauge key={g.id} gauge={g} />
        ))}
      </div>
    </div>
  );
