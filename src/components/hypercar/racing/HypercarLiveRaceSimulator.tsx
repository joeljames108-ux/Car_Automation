...
export const HypercarLiveRaceSimulator: React.FC<HypercarLiveRaceSimulatorProps> = ({
  ...
  // Day / Night Cycle (0: Day, 1: Dusk, 2: Night, 3: Dawn)
  ...
  const handleExecutePitStop = () => {
    ...
    setTimeout(() => {
      ...
      setPitStopMessage("PIT STOP COMPLETE (31.4s) — Rejoining Track!");
  ...
  }, [isPlaying, playbackSpeed, raceFinished, totalSimLaps, totalHours, setup]);

  return (
    <div className="w-full h-full flex flex-col bg-[#05070a] text-white select-none relative overflow-hidden">
      {/* Top Session Header */}
      <div className="p-4 bg-black/70 border-b border-white/10 flex items-center justify-between backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider text-white">
                {circuit.name} — WEC Endurance Session
              </h2>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/60 border border-white/10 text-xs font-mono">
            {isNightTime ? (
              <span className="text-cyan-400 flex items-center gap-1">
                <Moon className="w-3.5 h-3.5" /> NIGHT STINT
              </span>
            ) : (
              <span className="text-amber-400 flex items-center gap-1">
                <Sun className="w-3.5 h-3.5" /> DAY STINT
              </span>
            )}
          </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 p-6 overflow-y-auto">
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 rounded-xl border flex items-center justify-between text-xs font-mono transition-all ${
                      isPlayer
                        ? "bg-amber-500/20 border-amber-500/80 shadow-md shadow-amber-500/10 text-white font-bold"
        ...
        <div className="col-span-8 space-y-4">
          {/* Large Speedometer & e-AWD Hybrid State */}
          <div className="p-6 rounded-2xl bg-zinc-900/60 border border-white/10 relative overflow-hidden">
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col items-end gap-1.5">
                <span className="text-[10px] font-mono text-zinc-400">FRONT e-AWD MGU STATUS</span>
                <div
          ...
          <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/10 space-y-3">
            <h4 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
              ...
              Endurance Stint Reliability & Diagnostics
            </h4>
            <div className="grid grid-cols-3 gap-3 text-xs font-mono">
              ...
              <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                <span className="text-[10px] text-zinc-500 block">TIRE STINT TARGET</span>
                <span className="font-bold text-amber-400">{setup.tireCompound.replace("_", " ")}</span>
              </div>
              <div className="p-3 rounded-xl bg-black/40 border border-white/5">
                <span className="text-[10px] text-zinc-500 block">CHASSIS INTEGRITY</span>
                <span className="font-bold text-white">100% Structural Rigidity</span>
              </div>
            </div>
      ...
      {raceFinished && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4">
          <div className="max-w-md w-full bg-zinc-900 border border-amber-500/50 rounded-2xl p-6 shadow-2xl space-y-4 text-white text-center">
            ...
            <p className="text-xs text-zinc-400">
              Your custom constructed Hypercar completed the endurance distance with 0 mechanical failures.
            </p>
            ...
            <div className="p-4 rounded-xl bg-black/60 border border-white/10 text-xs font-mono space-y-2 text-left">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-emerald-400 font-bold">{playerPosition === 1 ? "+25 PTS" : playerPosition === 2 ? "+18 PTS" : "+15 PTS"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-500">HOMOLOGATED CAR:</span>
