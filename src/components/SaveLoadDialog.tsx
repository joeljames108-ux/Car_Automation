    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-base-900 border border-base-700 rounded-xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-base-800">
          <h2 className="text-sm font-semibold text-amber-50 flex items-center gap-2">
            {mode === "save" ? <><Save size={16} /> Save Design</> : <><FolderOpen size={16} /> Load Design</>}
          </h2>
          <button onClick={onClose} className="text-amber-300/50 hover:text-amber-100/80"><X size={18} /></button>
        </div>
        ...
        {mode === "save" ? (
          <div className="p-4 space-y-3">
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-base-850 border border-base-700 rounded-lg px-3 py-2 text-sm text-amber-50 focus:border-accent-500 focus:outline-none" />
            </div>
            <div>
              <label className="label-mono mb-1.5 block">Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full bg-base-850 border border-base-700 rounded-lg px-3 py-2 text-sm text-amber-50 focus:border-accent-500 focus:outline-none resize-none" />
            </div>
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="p-3 border-b border-base-800">
              <div className="relative">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search designs..." className="w-full bg-base-850 border border-base-700 rounded-lg pl-9 pr-3 py-2 text-sm text-amber-50 focus:border-accent-500 focus:outline-none" />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              {loading && <p className="text-xs text-amber-300/50 text-center py-4">Loading...</p>}
              {!loading && filtered.length === 0 && <p className="text-xs text-amber-300/50 text-center py-4">No designs found</p>}
              {filtered.map((d) => (
                <div key={d.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-base-850 border border-base-800 mb-1">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-amber-400">{d.description || "No description"}</div>
                  </div>
                  <button onClick={() => loadDesign(d)} className="text-accent-400 hover:text-accent-300 text-xs">Load</button>
                  <button onClick={() => cloneDesign(d)} className="text-amber-300/50 hover:text-amber-100/80"><Copy size={14} /></button>
