    ...
    <div className="space-y-4 p-4 rounded-2xl bg-amber-950/80 border border-amber-500/30 backdrop-blur-xl shadow-2xl text-xs font-mono">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl bg-amber-950/30 border border-amber-500/40">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-extrabold text-amber-50">CABIN ENGINEERING COMPARISON BENCH</h3>
        ...
        <div className="flex items-center gap-2">
          <span className="text-amber-200/60">BENCHMARK CABIN:</span>
          <select
            value={benchmarkKey}
            onChange={(e) => setBenchmarkKey(e.target.value)}
            className="px-3 py-1.5 rounded-xl bg-amber-900/50 border border-amber-500/40 text-amber-300 font-bold outline-none"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Cabin B Card */}
        <div className="p-4 rounded-xl bg-amber-900/40 border border-amber-500/30 space-y-3">
          <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="p-2 rounded-lg bg-amber-950/80 border border-amber-800/30">
              <span className="block text-amber-200/60">Total Mass:</span>
              <span className="font-extrabold text-amber-50">{benchmarkCabin.metrics.totalInteriorMassKg} kg</span>
            </div>
            <div className="p-2 rounded-lg bg-amber-950/80 border border-amber-800/30">
              <span className="block text-amber-200/60">BOM Cost:</span>
              <span className="font-extrabold text-amber-50">${benchmarkCabin.metrics.totalInteriorCostUSD.toLocaleString()}</span>
            </div>
            <div className="p-2 rounded-lg bg-amber-950/80 border border-amber-800/30">
              <span className="block text-amber-200/60">Ear Noise Level:</span>
              <span className="font-extrabold text-amber-400">{acousticB.driverEarSplDba} dBA</span>
            </div>
            <div className="p-2 rounded-lg bg-amber-950/80 border border-amber-800/30">
              <span className="block text-amber-200/60">SAE Ergo Score:</span>
      ...
      <div className="p-4 rounded-xl bg-amber-900/40 border border-amber-800/30 space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3 rounded-lg bg-amber-950/80 border border-amber-800/30 text-center">
            <span className="text-amber-200/60 text-[10px] block">MASS DELTA</span>
          ...
          <div className="p-3 rounded-lg bg-amber-950/80 border border-amber-800/30 text-center">
            <span className="text-amber-200/60 text-[10px] block">COST DELTA</span>
          ...
          <div className="p-3 rounded-lg bg-amber-950/80 border border-amber-800/30 text-center">
            <span className="text-amber-200/60 text-[10px] block">COMFORT DELTA</span>
            <span className={`text-base font-extrabold ${delta.comfortDiffPercent > 0 ? "text-emerald-400" : "text-amber-200/60"}`}>
              {delta.comfortDiffPercent > 0 ? `+${delta.comfortDiffPercent}%` : `${delta.comfortDiffPercent}%`}
            </span>
          </div>

          <div className="p-3 rounded-lg bg-amber-950/80 border border-amber-800/30 text-center">
            <span className="text-amber-200/60 text-[10px] block">LATERAL G SUPPORT</span>
            <span className={`text-base font-extrabold ${delta.lateralGSupportDiff > 0 ? "text-emerald-400" : "text-amber-200/60"}`}>
