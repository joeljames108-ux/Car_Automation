...
export const InteriorConfigControls: React.FC = () => {
  ...
  return (
    <div className="idash-panel-right bg-[#0d121f] text-slate-100 border-l border-slate-800 p-4 flex flex-col gap-3 overflow-y-auto w-[340px] flex-shrink-0">
      {/* Header */}
      <div className="flex items-center justify-between pb-1 border-b border-slate-800/80">
        ...
        <button
          className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-red-600/80 text-slate-200 hover:text-white border border-slate-700 hover:border-red-500 text-[11px] font-mono font-bold transition-all shadow-sm active:scale-95 cursor-pointer"
          onClick={reset}
          type="button"
      ...
      <div className="flex flex-col gap-2">
        {FEATURE_ORDER.map((key) => {
          ...
          return (
            <div
              key={key}
              className="flex items-center justify-between p-2 rounded-xl bg-slate-900/90 hover:bg-slate-800/90 border border-slate-800 hover:border-cyan-500/50 transition-all shadow-sm group"
            >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  className="w-7 h-7 rounded-lg bg-slate-800 hover:bg-blue-600 active:bg-blue-700 border border-slate-700 hover:border-cyan-400 text-white font-extrabold text-base flex items-center justify-center transition-all shadow-sm active:scale-90 cursor-pointer"
                ...
                <button
                  className="w-6 h-6 rounded-lg text-slate-400 hover:text-cyan-300 hover:bg-slate-800 transition-all flex items-center justify-center cursor-pointer text-xs"
      ...
      <div className="mt-2 pt-2 border-t border-slate-800/80 flex flex-col gap-2">
        ...
        {showCustomPicker ? (
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-900 border border-cyan-500/40 animate-in fade-in duration-150">
            <input
              type="color"
              value={interiorColor}
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-400">Custom PBR Leather Finish</span>
            </div>
          </div>
        ) : (
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-xl cursor-pointer transition-all duration-200 border-2 shadow-sm ${
                    isSelected
                      ...
                      : "border-slate-700/60 hover:border-slate-400 hover:scale-105"
                  }`}
      ...
      {selectedInfoKey && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-in fade-in duration-150">
          <div className="bg-slate-900 border border-cyan-500/40 rounded-2xl p-5 max-w-sm w-full shadow-[0_0_40px_rgba(0,0,0,0.9)] space-y-3.5">
