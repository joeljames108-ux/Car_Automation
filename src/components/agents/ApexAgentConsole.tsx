...
export function ApexAgentConsole({
  ...
  return (
    <div
      className={`w-full bg-[#0b0f19]/90 border border-amber-800/30 rounded-3xl p-5 backdrop-blur-2xl shadow-[0_20px_60px_rgba(0,0,0,0.45)] text-amber-50 select-none space-y-4 ${className}`}
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-amber-800/40 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-extrabold font-mono uppercase tracking-wider text-amber-50">
        ...
        <div className="flex items-center gap-1.5 bg-amber-950/75 p-1 rounded-2xl border border-amber-800/30">
          <button
      ...
      {activeMode === "powertrain" && (
        <div className="space-y-4">
              ...
              <button
                ...
                className={`p-3 rounded-2xl border text-left transition-all cursor-pointer ${
                  selectedPreset === p.id
                    ...
                    : "bg-amber-950/60 border-amber-800/30 text-amber-200/60 hover:bg-amber-900/50 hover:text-amber-50"
                }`}
              >
                <div className="text-xs font-mono font-extrabold">{p.label}</div>
                <div className="text-[10px] text-amber-300/50 font-medium mt-0.5">{p.desc}</div>
              </button>
            ))}
          </div>

          {/* Selected Tuning Card & One-Click Apply */}
          <div className="p-4 rounded-2xl bg-amber-950/75 border border-amber-800/30 flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-1 min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <Sparkles size={16} className="text-amber-400 animate-spin" />
                <h4 className="text-sm font-bold text-amber-50">{recommendation.title}</h4>
                <span
              ...
              <p className="text-xs text-amber-200/60 font-medium">{recommendation.summary}</p>
          ...
          <div className="p-3.5 rounded-2xl bg-amber-950/60 border border-amber-800/40 space-y-2">
            <div className="text-xs font-mono font-extrabold text-amber-100/80 uppercase tracking-wider flex items-center gap-1.5">
              <Activity size={14} className="text-amber-400" /> Chief Powertrain Live Diagnostics
            </div>
            <div className="space-y-1.5">
              {diagnostics.map((diag, idx) => (
                <div key={idx} className="text-xs font-mono text-amber-100/80 bg-amber-900/40 p-2 rounded-xl border border-amber-800/30">
                  {diag}
                </div>
              ))}
            </div>
      ...
      {activeMode === "assembly_qa" && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="p-4 rounded-2xl bg-amber-950/75 border border-amber-800/30 text-center">
              <span className="text-[10px] font-mono text-amber-200/60 uppercase tracking-widest block">Quality Score</span>
              <span className="text-2xl font-mono font-extrabold text-emerald-400 mt-1 block">{qaReport.qualityScore}%</span>
            </div>
            <div className="p-4 rounded-2xl bg-amber-950/75 border border-amber-800/30 text-center">
              <span className="text-[10px] font-mono text-amber-200/60 uppercase tracking-widest block">Torque Verification</span>
              <span className="text-2xl font-mono font-extrabold text-amber-400 uppercase mt-1 block">{qaReport.torqueVerification}</span>
            </div>
            <div className="p-4 rounded-2xl bg-amber-950/75 border border-amber-800/30 text-center">
              <span className="text-[10px] font-mono text-amber-200/60 uppercase tracking-widest block">Deck Clearance</span>
              <span className="text-2xl font-mono font-extrabold text-amber-400 mt-1 block">{qaReport.deckClearanceMm} mm</span>
            </div>
          </div>

          <div className="p-4 rounded-2xl bg-amber-950/75 border border-amber-800/30 space-y-2">
            <h4 className="text-xs font-mono font-bold text-amber-50 uppercase tracking-wider flex items-center gap-2">
              <ShieldCheck size={16} className="text-emerald-400" /> Robotic Inspection Checklist
            </h4>
            <div className="space-y-1.5">
              {qaReport.insights.map((insight, idx) => (
                <div key={idx} className="text-xs font-mono text-amber-100/80 bg-amber-900/40 p-2.5 rounded-xl border border-amber-800/30">
                  {insight}
                </div>
              ))}
            </div>
      ...
      {activeMode === "race_strategy" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {circuitPredictions.map((cp, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-amber-950/75 border border-amber-800/30 space-y-2.5">
              <div className="flex items-center justify-between border-b border-amber-800/30 pb-2">
                <span className="text-xs font-mono font-extrabold text-amber-50 truncate">{cp.circuitName}</span>
                <Flag size={14} className="text-amber-400 shrink-0" />
              </div>
              <div className="space-y-1.5 font-mono text-xs">
                <div className="flex justify-between">
                  <span className="text-amber-200/60">Predicted Lap:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-amber-200/60">Fuel / Lap:</span>
                  <span className="text-emerald-400 font-extrabold">{cp.fuelBurnLitersPerLap} L</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-amber-200/60">Tire Wear / Lap:</span>
      ...
      {activeMode === "chat" && (
        <div className="space-y-3">
          <div className="h-44 bg-amber-950/85 border border-amber-800/30 rounded-2xl p-3 overflow-y-auto space-y-2 font-mono text-xs">
              ...
              <div
                ...
                className={`p-2.5 rounded-xl max-w-[85%] ${
                  msg.sender === "user"
                    ...
                    : "bg-amber-900/50 text-amber-50 border border-amber-800/30 mr-auto"
                }`}
              >
                {msg.text}
              </div>
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-amber-950/80 border border-amber-800/30 rounded-xl px-3.5 py-2 text-xs font-mono text-amber-50 focus:outline-none focus:border-amber-400 transition-colors"
            />
            <button
              type="submit"
