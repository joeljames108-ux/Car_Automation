    ...
    <div className="panel p-3.5 rounded-2xl space-y-3.5 border border-base-800 text-xs font-mono shadow-xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div className="p-2 rounded-xl bg-base-900/60 border border-base-800 text-center">
          <span className="text-[9px] text-amber-300/50 block uppercase">STRUCTURAL RIGIDITY</span>
          <span className="font-bold text-amber-50 text-xs">{healthReport.structuralIntegrityScore}%</span>
        </div>
        <div className="p-2 rounded-xl bg-base-900/60 border border-base-800 text-center">
          <span className="text-[9px] text-amber-300/50 block uppercase">CLEARANCE MARGIN</span>
          <span className="font-bold text-amber-300 text-xs">{healthReport.packagingClearanceScore}%</span>
        </div>
        <div className="p-2 rounded-xl bg-base-900/60 border border-base-800 text-center">
          <span className="text-[9px] text-amber-300/50 block uppercase">THERMAL CAPACITY</span>
          <span className="font-bold text-amber-300 text-xs">{healthReport.thermalSafetyScore}%</span>
        </div>
        <div className="p-2 rounded-xl bg-base-900/60 border border-base-800 text-center">
          <span className="text-[9px] text-amber-300/50 block uppercase">AERO STABILITY</span>
      ...
      <div className="p-2.5 rounded-xl bg-base-900/80 border border-base-800 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-amber-100/80 font-bold text-[10px]">
          ...
          <button
            ...
            className={`px-2 py-0.5 rounded-lg text-[9px] font-bold border transition-all cursor-pointer ${
              showCoMGizmo
                ...
                : "bg-base-850 border-base-700 text-amber-300/50 hover:text-amber-100/80"
            }`}
          >
      ...
      <div className="space-y-1.5 max-h-[160px] overflow-y-auto no-scrollbar pr-1">
        <span className="text-[10px] font-bold text-amber-200/60 block uppercase tracking-wider">
          ...
          return (
            <div
              ...
              className={`p-2 rounded-xl border flex items-start gap-2 transition-all cursor-pointer ${
                isConflict
                  ...
                  : isPass
                  ? "bg-base-900/40 border-base-800/60 text-amber-200/60"
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-[10px] text-amber-50 truncate">{issue.title}</span>
                  {issue.clearanceMm !== undefined && (
                    <span className="text-[9px] font-mono text-amber-400 shrink-0">{issue.clearanceMm}mm</span>
                  )}
                </div>
                <p className="text-[9px] text-amber-200/60 leading-snug line-clamp-2 mt-0.5">{issue.description}</p>
