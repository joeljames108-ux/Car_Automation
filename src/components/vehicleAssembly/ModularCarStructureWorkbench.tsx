...
export const ModularCarStructureWorkbench: React.FC<ModularCarStructureWorkbenchProps> = ({
  ...
  const getGradeColor = (grade: MaterialGrade) => {
    switch (grade) {
      ...
      default:
        return 'text-amber-200/60 border-amber-700/30 bg-amber-900/40';
    }
  };
    ...
    <div className="bg-gradient-to-b from-slate-950/90 via-slate-900/80 to-slate-950/95 border border-amber-500/30 rounded-2xl p-4 shadow-2xl backdrop-blur-xl space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-amber-500/20">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="bg-amber-900/40 border border-amber-700/30 text-amber-300 text-xs rounded-lg px-2 py-1 font-mono focus:outline-none focus:border-amber-500"
          >
      ...
      {activeTab === 'hierarchy' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-2 bg-amber-900/40 border border-amber-800/30 rounded-xl p-3 max-h-[460px] overflow-y-auto">
            <div className="flex items-center justify-between text-xs text-amber-200/60 font-mono pb-2 border-b border-amber-800/30">
              ...
              return (
                <div key={category} className="space-y-1">
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2 rounded-lg bg-amber-800/35/40 hover:bg-amber-800/35/70 border border-amber-700/30/40 text-xs font-mono text-left transition-all"
                  >
                    <div className="flex items-center gap-2">
                      {isExpanded ? <ChevronDown size={14} className="text-amber-400" /> : <ChevronRight size={14} className="text-amber-200/60" />}
                      <span className="font-bold text-amber-50 uppercase tracking-wider">{category} ASSEMBLY</span>
                      <span className="text-[10px] text-amber-300/50">({categoryNodes.length} nodes)</span>
                    </div>
                    ...
                    <div className="pl-4 space-y-1">
                      {categoryNodes.map((node) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <CornerDownRight size={12} className={isSelected ? 'text-amber-400' : 'text-amber-400'} />
          ...
          <div className="bg-amber-900/40 border border-amber-500/30 rounded-xl p-3.5 space-y-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-amber-50">{selectedNode.name}</h4>
              <p className="text-xs text-amber-200/60 mt-1">{selectedNode.description}</p>
            </div>

            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              <div className="bg-amber-950/60 p-2 rounded-lg border border-amber-800/30">
                <span className="text-[10px] text-amber-300/50 block">BASE MASS</span>
                <strong className="text-amber-300 text-sm">{Math.round(selectedNode.baseMassKg)} kg</strong>
              </div>
              <div className="bg-amber-950/60 p-2 rounded-lg border border-amber-800/30">
                <span className="text-[10px] text-amber-300/50 block">MASS TYPE</span>
              ...
              <div className="bg-amber-950/60 p-2 rounded-lg border border-amber-800/30">
                <span className="text-[10px] text-amber-300/50 block">METALLURGY GRADE</span>
                <strong className="text-emerald-300 text-sm">{selectedNode.materialGrade.toUpperCase()}</strong>
              </div>
              <div className="bg-amber-950/60 p-2 rounded-lg border border-amber-800/30">
                <span className="text-[10px] text-amber-300/50 block">JOINT STIFFNESS</span>
            ...
            <div className="bg-amber-950/80 p-2.5 rounded-lg border border-amber-800/30 space-y-1 text-xs font-mono">
              <span className="text-[10px] text-amber-300/50 block font-bold">LOCAL 3D CENTER OF MASS [X, Y, Z]</span>
              <div className="grid grid-cols-3 gap-1 text-center text-amber-100/80">
                <div className="bg-amber-900/50 p-1 rounded border border-amber-800/30">
      ...
      {activeTab === 'cog_mass' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-amber-900/40 border border-amber-800/30 rounded-xl p-3.5 space-y-3 font-mono">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-amber-200/60">Total Vehicle Curb Weight</span>
                <strong className="text-amber-300 font-bold">{telemetry.totalMassKg} kg</strong>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-amber-200/60">Sprung Mass</span>
                <span className="text-amber-300 font-semibold">{telemetry.sprungMassKg} kg</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-amber-200/60">Unsprung Mass (Corners)</span>
                <span className="text-amber-300 font-semibold">{telemetry.unsprungMassKg} kg</span>
              </div>
            </div>

            <div className="pt-2 border-t border-amber-800/30 space-y-1.5 text-xs">
              <span className="text-[10px] text-amber-300/50 font-bold block">3D CENTER OF GRAVITY COORDINATES</span>
              <div className="grid grid-cols-3 gap-1 text-center">
                <div className="bg-amber-950/80 p-1.5 rounded border border-amber-800/30">
                  <span className="text-[9px] text-amber-300/50 block">Xcg (Aft Fr Axle)</span>
                  <strong className="text-amber-300">{Math.abs(telemetry.centerOfGravity.xMm)} mm</strong>
                </div>
                <div className="bg-amber-950/80 p-1.5 rounded border border-amber-800/30">
                  <span className="text-[9px] text-amber-300/50 block">Ycg (Height)</span>
                  <strong className="text-amber-300">{telemetry.centerOfGravity.yMm} mm</strong>
                </div>
                <div className="bg-amber-950/80 p-1.5 rounded border border-amber-800/30">
                  <span className="text-[9px] text-amber-300/50 block">Zcg (Lateral)</span>
                  <strong className="text-amber-100/80">{telemetry.centerOfGravity.zMm} mm</strong>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-amber-800/30 space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-amber-200/60 font-bold">FRONT {telemetry.weightDistribution.frontPercent}%</span>
                <span className="text-amber-200/60 font-bold">REAR {telemetry.weightDistribution.rearPercent}%</span>
              </div>
              <div className="w-full h-3 bg-amber-950/80 rounded-full overflow-hidden flex border border-amber-700/30">
          ...
          <div className="bg-amber-900/40 border border-amber-800/30 rounded-xl p-3.5 space-y-3 font-mono">
            ...
            <div className="grid grid-cols-2 gap-2 text-center text-xs">
              {/* Front Left */}
              <div className="bg-amber-950/80 p-2.5 rounded-lg border border-amber-500/40 space-y-0.5">
                ...
                <span className="text-[10px] text-amber-300/50 block">{telemetry.cornerForcesN.fl} N</span>
              </div>
              ...
              <div className="bg-amber-950/80 p-2.5 rounded-lg border border-amber-500/40 space-y-0.5">
                ...
                <span className="text-[10px] text-amber-300/50 block">{telemetry.cornerForcesN.fr} N</span>
              </div>
              ...
              <div className="bg-amber-950/80 p-2.5 rounded-lg border border-amber-500/40 space-y-0.5">
                ...
                <span className="text-[10px] text-amber-300/50 block">{telemetry.cornerForcesN.rl} N</span>
              </div>
              ...
              <div className="bg-amber-950/80 p-2.5 rounded-lg border border-amber-500/40 space-y-0.5">
                ...
                <span className="text-[10px] text-amber-300/50 block">{telemetry.cornerForcesN.rr} N</span>
              </div>
            </div>

            <div className="pt-2 border-t border-amber-800/30 flex justify-between text-xs">
              <span className="text-amber-200/60">Cross-Weight (FL + RR):</span>
          ...
          <div className="bg-amber-900/40 border border-amber-800/30 rounded-xl p-3.5 space-y-3 font-mono">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between">
                <span className="text-amber-200/60">Front Roll Center Height</span>
                <strong className="text-amber-300">{telemetry.kinematics.frontRollCenterHeightMm} mm</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-amber-200/60">Rear Roll Center Height</span>
                <strong className="text-amber-300">{telemetry.kinematics.rearRollCenterHeightMm} mm</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-amber-200/60">Roll Axis Inclination</span>
                <strong className="text-yellow-300">+{telemetry.kinematics.rollAxisInclinationDeg}°</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-amber-200/60">Yaw Inertia (Izz)</span>
                <strong className="text-amber-50">{telemetry.kinematics.yawMomentOfInertiaIzz} kg·m²</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-amber-200/60">Pitch Inertia (Iyy)</span>
                <strong className="text-amber-50">{telemetry.kinematics.pitchMomentOfInertiaIyy} kg·m²</strong>
              </div>
            </div>

            <div className="p-2.5 rounded-lg bg-amber-950/80 border border-amber-800/30 text-[11px] text-amber-200/60">
      ...
      {activeTab === 'fea_rigidity' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono">
          ...
          <div className="bg-amber-900/40 border border-amber-800/30 rounded-xl p-3.5 space-y-3">
            <h4 className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
              <Flame size={14} />
          ...
          <div className="md:col-span-2 bg-amber-900/40 border border-amber-800/30 rounded-xl p-3.5 space-y-2">
            <div className="flex items-center justify-between pb-2 border-b border-amber-800/30 text-xs">
              <span className="font-bold text-amber-300">FEA STRUCTURAL STRESS HOTSPOT NODES</span>
              <span className="text-amber-300/50">2G CORNERING / 3G BUMP LOAD CASES</span>
            </div>
            ...
            <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
              {telemetry.feaHotspots.map((spot) => (
                <div
                  key={spot.nodeId}
                  className="flex items-center justify-between p-2 rounded-lg bg-amber-950/60 border border-amber-800/30 text-xs"
                >
                  <div>
                    <span className="font-bold text-amber-50 block">{spot.name}</span>
                  ...
                  <div className="flex items-center gap-4 text-right">
                    <div>
                      ...
                      <span className="text-[10px] text-amber-300/50 block">Yield: {spot.yieldStrengthMpa} MPa</span>
                    </div>
      ...
      {activeTab === 'hardpoints' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 font-mono text-xs">
          {/* Wheelbase */}
          <div className="bg-amber-900/40 p-3 rounded-xl border border-amber-800/30 space-y-2">
            <div className="flex justify-between">
              <span className="text-amber-200/60">Wheelbase</span>
            ...
            />
            <span className="text-[10px] text-amber-300/50 block">Front-to-rear axle datum span</span>
          </div>

          {/* Front Track */}
          <div className="bg-amber-900/40 p-3 rounded-xl border border-amber-800/30 space-y-2">
            <div className="flex justify-between">
              <span className="text-amber-200/60">Front Track Width</span>
            ...
            <input
              ...
              className="w-full accent-amber-500 cursor-pointer"
            />
            <span className="text-[10px] text-amber-300/50 block">Front tire centerline lateral span</span>
          </div>

          {/* Rear Track */}
          <div className="bg-amber-900/40 p-3 rounded-xl border border-amber-800/30 space-y-2">
            <div className="flex justify-between">
              <span className="text-amber-200/60">Rear Track Width</span>
            ...
            <input
              ...
              className="w-full accent-amber-500 cursor-pointer"
            />
            <span className="text-[10px] text-amber-300/50 block">Rear tire centerline lateral span</span>
          </div>

          {/* Ride Height */}
          <div className="bg-amber-900/40 p-3 rounded-xl border border-amber-800/30 space-y-2">
            <div className="flex justify-between">
              <span className="text-amber-200/60">Ride Height</span>
            ...
            <input
              ...
              className="w-full accent-amber-500 cursor-pointer"
            />
            <span className="text-[10px] text-amber-300/50 block">Ground clearance to chassis underside</span>
          </div>
        </div>
      )}
    </div>
