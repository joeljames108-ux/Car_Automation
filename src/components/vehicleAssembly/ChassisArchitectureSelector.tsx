          ...
          return (
            <div
              ...
              className={`p-3.5 rounded-2xl border cursor-pointer transition-all duration-200 flex flex-col justify-between space-y-2 select-none ${
                isSelected
                  ? 'bg-amber-500/10 dark:bg-amber-950/40 border-amber-500 dark:border-amber-400 shadow-[0_0_15px_rgba(6,182,212,0.3)] scale-102 ring-1 ring-amber-400'
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${isSelected ? 'bg-amber-500 text-slate-950' : 'bg-slate-200 dark:bg-base-900 text-amber-300/50'}`}>
                    PLATFORM #{idx + 1}
                  </span>
                  {isSelected && <CheckCircle2 size={15} className="text-amber-400" />}
                </div>

                <h4 className="text-xs font-bold text-slate-800 dark:text-amber-50 leading-tight">
                  {chassis.name}
                </h4>
                <p className="text-[10px] text-amber-300/50 dark:text-amber-200/60 mt-1 line-clamp-2">
                  {chassis.tagline}
                </p>
              </div>

              {/* Spec Mini Badges */}
              <div className="space-y-1 pt-2 border-t border-slate-200 dark:border-amber-800/30 text-[10px]">
                <div className="flex justify-between">
                  <span className="text-amber-300/50">Mass:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-amber-300/50">BOM:</span>
