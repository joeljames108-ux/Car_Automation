...
export const ExteriorCompletionModal: React.FC<ExteriorCompletionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-fadeIn">
      <div className="relative max-w-lg w-full bg-slate-900 border border-cyan-500/40 rounded-3xl p-6 shadow-2xl space-y-5 text-center">
        <div className="w-16 h-16 rounded-3xl bg-cyan-500/20 border border-cyan-500/40 text-cyan-300 flex items-center justify-center mx-auto">
          <Trophy size={32} />
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 p-3 rounded-2xl bg-slate-950 border border-white/10 font-mono text-xs text-center">
          ...
          <div>
            ...
            <strong className="text-cyan-400 font-bold">{totalDft} µm</strong>
          </div>
          <div>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-2xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-mono font-bold text-xs shadow-lg hover:shadow-[0_0_15px_rgba(6,182,212,0.5)] transition-all"
          >
            CONTINUE INSPECTION
          </button>
        </div>
