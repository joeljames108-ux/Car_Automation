  ...
  return (
    <div className="space-y-6 text-amber-50">
      ...
      <div className="bg-amber-900/40 backdrop-blur-xl p-6 rounded-2xl border border-amber-800/30 flex items-center justify-between shadow-xl">
        <div>
          <h2 className="text-xl font-black tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-500">
        ...
        <button
          ...
          className="px-5 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-cyan-500/20 flex items-center space-x-2 transition-all active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          ...
          <div className="text-[11px] text-amber-300/50 mt-1">Daily Freight: ${lastTickResult.dailyLogisticsCostUSD.toLocaleString()}</div>
        </div>
        ...
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          ...
          <div className="text-[11px] text-amber-300/50 mt-1">PPM Defect Chargebacks: ${lastTickResult.warrantyChargebackUSD}</div>
        </div>
        ...
        <div className="bg-amber-900/40 p-4 rounded-xl border border-amber-800/30">
          ...
          <div className="text-[11px] text-amber-300/50 mt-1">{lastTickResult.activeAlerts.length} active alerts logged</div>
        </div>
      </div>

      {/* Raw Materials Market Spot Quotes */}
      <div className="bg-amber-900/40 p-5 rounded-2xl border border-amber-800/30 space-y-3">
        <h3 className="text-sm font-bold text-amber-50 flex items-center space-x-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {commodities.map((cKey) => {
            ...
            return (
              <div key={cKey} className="bg-amber-950/80 p-3 rounded-xl border border-amber-800/30">
                <div className="text-[10px] text-amber-200/60 font-bold truncate">{quote.name}</div>
                <div className="text-sm font-mono font-black text-white mt-1">
                  ${quote.spotPriceUSD.toFixed(2)} <span className="text-[10px] text-amber-300/50">/{quote.unitOfMeasure}</span>
                </div>
      ...
      <div className="bg-amber-900/40 p-5 rounded-2xl border border-amber-800/30 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-amber-50">TIER-1 & TIER-2 SUPPLIER DIRECTORY</h3>
          <div className="flex items-center space-x-2">
            <span className="text-xs text-amber-200/60">Category Filter:</span>
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="bg-amber-950/80 text-xs text-amber-50 px-3 py-1.5 rounded-lg border border-amber-700/30 outline-none"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSuppliers.map((s) => (
            <div key={s.id} className="bg-amber-950/80 p-4 rounded-xl border border-amber-800/30 space-y-2">
              <div className="flex items-start justify-between">
                <div>
                  <h4 className="text-xs font-bold text-white">{s.name}</h4>
                  <div className="text-[10px] text-amber-200/60">{s.country} • {s.category}</div>
                </div>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 font-bold border border-amber-500/30">
                  {s.costMultiplier}x Cost
                </span>
              </div>
              <p className="text-[11px] text-amber-200/60 line-clamp-2">{s.specialtyDescription}</p>
              <div className="flex items-center justify-between text-[10px] text-amber-300/50 pt-2 border-t border-amber-800/30 font-mono">
