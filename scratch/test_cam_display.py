...
# Three.js: pos = (-0.15, 0.70, 0.42), target = (0.01, 0.58, 0.08), fov = 44
# Blender: loc = (-0.15, -0.42, 0.70), target = (0.01, -0.08, 0.58)
cam_loc = Vector((-0.15, -0.42, 0.70))
cam_target = Vector((0.01, -0.08, 0.58))
cam_obj.location = cam_loc
dir_vec = (cam_target - cam_loc).normalized()
cam_obj.rotation_euler = dir_vec.to_track_quat('-Z', 'Y').to_euler()
f = 36.0 / (2.0 * math.tan(math.radians(44) / 2.0))
cam_data.lens = f

scene.render.filepath = os.path.join(out_dir, "test_cam_display_current.png")
bpy.ops.render.render(write_still=True)
print("Rendered test_cam_display_current.png")
