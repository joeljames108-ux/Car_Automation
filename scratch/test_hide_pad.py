...
# Hide DASH_UPPER_PAD
pad = bpy.data.objects.get("DASH_UPPER_PAD")
if pad:
    pad.hide_render = True
    print("DASH_UPPER_PAD hidden!")
...
cam_loc = Vector((-0.38, -0.22, 0.75))
cam_target = Vector((-0.38, -0.078, 0.735))
cam_obj.location = cam_loc
dir_vec = (cam_target - cam_loc).normalized()
cam_obj.rotation_euler = dir_vec.to_track_quat('-Z', 'Y').to_euler()
f = 36.0 / (2.0 * math.tan(math.radians(52) / 2.0))
cam_data.lens = f

scene.render.filepath = os.path.join(out_dir, "test_hide_pad.png")
bpy.ops.render.render(write_still=True)
print("Rendered test_hide_pad.png")
